<template>
  <div class="form form-inline-center">
    ...
    <el-row class="operate-row text-center">
      <el-button type="primary" @click="submitForm('user')">Save</el-button>
  ...
  export default {
    ...
    methods: {
      submitForm (formName) {
        ...
        ref[formName].validate((valid) => {
          if (valid) {
            ...
            vm.api.post("perm/user/editUser", vm.$qs.stringify(vm.user, { arrayFormat: 'repeat' }))
              .then((r) => {
                vm.loading = false;
                if (r.data.code === 0){
                  vm.$alert('编辑user成功', {
      ...
      editPageData (data) {
        ...
        this.user.confirmPwd = data.password;
        this.user.roleIdArr = [];
        data.roleList.forEach(item => {
          this.user.roleIdArr.push(item.id)
        })
      },
      getRoleListAndUserInfo () {
        let vm = this;
        vm.loading = true;
        vm.api.get("/perm/user/getRoleListAndUserInfo?id=" + vm.$route.query.id)
          .then((r) => {
            vm.loading = false;
            if (r.data.code === 0){
              vm.editPageData(r.data.resultMap.userInfo);
